...
def elasticDeform(imageVolume, maskVolume, sigma, alpha, interMethod='linear', extraMethod='nearest'):
    ...
    dx = 2 * np.random.rand(imageHeight, imageWidth, imageDepth) - 1  # ~U(-1, 1)
    dy = 2 * np.random.rand(imageHeight, imageWidth, imageDepth) - 1  # ~U(-1, 1)
    dz = 2 * np.random.rand(imageHeight, imageWidth, imageDepth) - 1  # ~U(-1, 1)

    # Smoothing and scaling the field
    kernelSize = 2 * np.ceil(2 * sigma) + 1  # square kernel size
    dx = alpha * gaussian_filter(dx, sigma, mode='constant', cval=0)  # filtered and scaled
    dy = alpha * gaussian_filter(dy, sigma, mode='constant', cval=0)  # filtered and scaled
    dz = alpha * gaussian_filter(dz, sigma, mode='constant', cval=0)  # filtered and scaled

    # Applying the random displacement (elastic distortion) to the pixels
    x, y, z = np.ogrid[:imageHeight, :imageWidth, :imageDepth]  # structured rectangular grid
    ...
    queryPoints_image = np.array([x + dx, y + dy, z + dz]).transpose((1, 2, 3, 0))  # query points for interpolation
    imageVolume_deformed = F_image(queryPoints_image)  # interpolant function evaluation at query points
    ...
    queryPoints_mask = np.array([x + dx, y + dy, z + dz]).transpose((1, 2, 3, 0))  # query points for interpolation
    maskVolume_deformed = F_mask(queryPoints_mask)  # interpolant function evaluation at query points
...
def biasField(imageVolume):
    ...
    imageHeight, imageWidth, imageDepth = imageVolume.shape  # volume size
    X, Y, Z = np.meshgrid(np.arange(1, imageHeight + 1), np.arange(1, imageWidth + 1), np.arange(1, imageDepth + 1), indexing='ij')  # structured rectangular grid
    x0 = np.random.randint(1, imageHeight + 1)  # random x-center
    y0 = np.random.randint(1, imageWidth + 1)  # random y-center
    z0 = np.random.randint(1, imageDepth + 1)  # random z-center
...
def motion_ghosting(imageVolume, alpha, numReps, p):
    ...
    if p == 1:  # along x-axis
        imageVolume[::numReps, :, :] = alpha * imageVolume[::numReps, :, :]  # k-space lines are modulated differently
    elif p == 2:  # along y-axis
        imageVolume[:, ::numReps, :] = alpha * imageVolume[:, ::numReps, :]  # k-space lines are modulated differently
    elif p == 3:  # along z-axis
        imageVolume[:, :, ::numReps] = alpha * imageVolume[:, :, ::numReps]  # k-space lines are modulated differently

    imageVolume = np.abs(ifftn(imageVolume, (imageHeight, imageWidth, imageDepth)))  # inverse ifft of modified data
